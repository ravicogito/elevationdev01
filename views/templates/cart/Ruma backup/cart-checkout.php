...
<div class="col-md-9">
	...
	<div>
		<?php 
		$failed_msg = $this->session->flashdata('failedMessage');
		if ($failed_msg != "") {
		?>
...
<?php //if(!empty($_SESSION['Usercart'])): ?>
	...
	<div class="col-md-3">
    <a href="<?php echo base_url(); ?>favourite" class="continue_shopbutt">Continue Shopping</a>
	<div class="sumary bdrsdo">

	<h3>Order Summary</h3>
	<!--<div class="sbtotl"><?//=count($photoArr[$userID])?> Photos<br/>-->
	<p>Order sub-total <strong>$<?=number_format($total_price, 2, ".","");?></strong></p>
    ...
    <form name="frmchkout" id="frmchkout" method="post" action="<?php echo base_url().'cartAdd/billingCheckout/';?>" autocomplete="off">
		...
		<!--<p>  Enter your destination to get a
		  ...
		  <?php if(!empty($_SESSION['Usercart'])){?> 
		   <input name="" id="chkoutSubmit" value="Buy" class="checkout_button" type="submit">
		  <?php }?>
		  </div>
