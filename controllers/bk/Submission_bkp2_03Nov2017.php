...
class Submission extends CI_Controller {
	...
	public function index() {		    
		...
		################### TO GET STATE LIST #############
        $cityDropdown = $this->model_registration->populateDropdown("city_id", "city_name", "anad_cities", "state_id = '".$userDetails['state_id']."'", "city_name", "ASC");
		$cityOptions = array();
		...
		//$this->elements['contentHtml']           	= $this->data['themePath'].'submission/final_submission';	
...
email: support@anandclub.org</p></td>
	...
	public function eventGallery()
	{
		...
		$this->data['event_category']				= $this->model_submission->getEventCategory();
		//echo "<pre>";
		//print_r($this->data['event_category']);
		...
		//print_r($this->data['event_gallery_info']);
		foreach($couponInfo as $key => $val){
		...
		$this->layout->view('templates/dashboard/event_gallery',$this->data);
	}
	...
	public function uploadimgGallery()
	{
		...
		$event_img_cat	= $this->input->post('event_img_cat');
		$this->data		= $event_img_cat;
		$user_id 		= $this->session->userdata('user_id'); 
		//print_r($_FILES);
		if($file_type == "image"){		

			if (isset($_FILES) && $_FILES['event_gallery_pic']['error'] == 0) {
				$filename = rand(000000000, 999999999) . time() . $_FILES['event_gallery_pic']['name'];
				...
				if (!file_exists('uploads/event_gallery')) {
					mkdir('uploads/event_gallery', 0777);
				}

				if (!$this->upload->do_upload('event_gallery_pic')) {
					$this->data['error'] = 1;
					$this->data['value'] = $this->upload->display_errors();
				} else {
					...
					$this->model_submission->insertEventImageGallery('anad_events_gallery',$insertEventGallery);
				}  
			}
		...
		redirect(base_url()."submission/eventGallery");
	}
