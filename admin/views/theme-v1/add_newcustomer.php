...
<script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
	...
	$('document').ready(function(){
		  ...
		  $.ajax({
			...
			success:function(responce) {
				...
				if(responce['process'] == 'success') {
					...
					$(".message").html('');
					//$(".box-button").css({display: "none"});
					$("#event_name").val(responce['event_name']);
					$('#event_name').attr('readonly', true);
					if(responce['event_price'] != ''){
					$("#event_price").val(responce['event_price']);
					$('#event_price').attr('readonly', true);
					}
					else{
						$("#event_price").hide();
					}
					...
					$('#customer_middlename').hide();					
					$('#customer_lastname').hide();
					$("#customer_email").val(responce['customer_email']);
					$('#customer_email').attr('readonly', true);
					$("#customer_mobile").val(responce['customer_mobile']);
					$('#customer_mobile').attr('readonly', true);
		...
		$("#location_id").unbind().bind("change", function() {
		  ...
		  $.ajax({
			url: "<?php echo base_url();?>customer/populateAjaxResortByLocation/",
	...
	$("#resort_id").unbind().bind("change", function() {
	  ...
	  var resortID     = $(this).val();
		//alert(resortID);
	  $.ajax({
		url: "<?php echo base_url();?>customer/populateAjaxPhotographerByLocation/",
                ...
                <div class="panel-body">
                    ...
                    <form role="form" method="post" action="<?php echo base_url().'Customer/addCustomer/';?>" onsubmit ="return Checkingform(this);" enctype="multipart/form-data">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" id="datepicker" name="event_date" placeholder="Enter Date">
						</div>
						<div class="form-group">
							<label>Event Price(in $)</label><br>
							<input type="test" id="event_price" name="event_price" placeholder="Put event price"  style="width:100%">
						</div>
						<div class="form-group">
						...
						<!--<div class="form-group">								
							...
							foreach ($resort_list as $vallist)
							{									
							?>									
						...
						<!--<div class="form-group">
							...
							<select name="photographer_id" id="photographer_name" class="form-control">
								<option value="">Select</option>
								<?php if(!empty($photographerlist)){ foreach ($photographerlist as $vallist){
								?>
								<option value="<?php echo $vallist->photographer_id; ?>"><?php echo $vallist->photographer_name; ?></option>
								<?php
								}
								} ?>
							</select>
